...
class Net(nn.Module):
    def __init__(self, data_dict):
        ...
        self.base_model = EfficientNet.from_pretrained('efficientnet-b0',in_channels=5) 
        ...
        #print(self.base_layers[0])
        self.layer0 = nn.Sequential(*self.base_layers[:2])#,*self.base_layers[2][0]) # size=(N, 64/16, x.H/2, x.W/2)
        self.layer0_1x1 = convrelu(32,32, 1, 0)
        self.layer1 = nn.Sequential(*self.base_layers[2][0:2]) # size=(N, 64/24, x.H/4, x.W/4)
        self.layer1_1x1 = convrelu(24, 24, 1, 0)
        self.layer2 = nn.Sequential(*self.base_layers[2][2:5]) # size=(N, 128/32, x.H/8, x.W/8)
        self.layer2_1x1 = convrelu(40, 40, 1, 0)
        self.layer3 = nn.Sequential(*self.base_layers[2][5:9])  # size=(N, 256/64, x.H/16, x.W/16)
        self.layer3_1x1 = convrelu(112, 112, 1, 0)
        self.layer4 = nn.Sequential(*self.base_layers[2][9:],*self.base_layers[3:5])  # size=(N, 512/1280, x.H/32, x.W/32)
        
        self.layer4_1x1 = convrelu(1280, 1280, 1, 0)

        self.upsample = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
        
        self.conv_up3 = convrelu(112 + 1280, 1280, 3, 1)
        self.conv_up2 = convrelu(40 + 1280, 512, 3, 1)
        self.conv_up1 = convrelu(24 + 512, 256, 3, 1)
        self.conv_up0 = convrelu(32 + 256, 128, 3, 1)
