...
class FireDeconv(nn.Module):
	def __init__(self, in_channels, c_1, c_2):
		...
		self.deconv = Deconv(c_1,c_1, kernel_size=5,stride=2,padding=1)
...
class FireDeconv_(nn.Module):
	def __init__(self, in_channels, c_1, c_2):
		...
		Args:
			...
			c_2 : parallel scaleup tp c_2 channels using 1x1 and 3x3 kernel
		"""
		super(FireDeconv_, self).__init__()
		self.layer_1 = Conv(in_channels,c_1,kernel_size=1,stride=1,padding=0)
		#self.deconv = Deconv(c_1,c_1, kernel_size=[1,4],stride=[1,2],padding=[0,1])
		self.deconv = Deconv(c_1,c_1, kernel_size=4,stride=2,padding=0)
...
class SqueezeNet(nn.Module):
	def __init__(self, NUM_CLASSES):
		...
		self.conv1 = Conv(3,64,3,stride=2,padding=0)
		...
		self.fire13 = FireDeconv_(64, 16, 32)
...
if __name__ == "__main__":
	import numpy as np
	x = torch.tensor(np.random.rand(2,3,360,480).astype(np.float32))
	model = SqueezeNet(NUM_CLASSES=9)
	y = model(x)
	print('output shape:', y.shape)
	assert y.shape == (2,9,224,224), 'output shape (2,9,64,512) is expected!'
	print('test ok!')