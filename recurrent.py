...
import utils.util.recurrent as util
...
class Recurrent(object):
	...
	def forward(self,x,mask,filters):
		...
		compatibility_kernel_init = torch.from_numpy(np.reshape(np.ones((data_dict.NUM_CLASSES, data_dict.NUM_CLASSES), dtype = "float32")
			-np.identity(data_dict.NUM_CLASSES, dtype="float32"),[data_dict.NUM_CLASSES, data_dict.NUM_CLASSES,1,1]))
